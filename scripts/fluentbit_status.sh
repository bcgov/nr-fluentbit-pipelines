...
if [ "${#SERVER_CONFIGS[@]}" -gt 0 ]; then
    for SERVER_CONFIG in ${SERVER_CONFIGS[@]} ; do
        ...
        export CD_USER=$(VAULT_ADDR=$VAULT_ADDR VAULT_TOKEN=$VAULT_TOKEN /sw_ux/bin/vault kv get -field=$FB_VAULT_CD_USER_FIELD $FB_VAULT_CD_PATH)
        export CD_PASS=$(VAULT_ADDR=$VAULT_ADDR VAULT_TOKEN=$VAULT_TOKEN /sw_ux/bin/vault kv get -field=$FB_VAULT_CD_PASS_FIELD $FB_VAULT_CD_PATH)

        if [ "$FB_RELEASE" == "null" ]; then
            export FB_RELEASE=$BASE_FB_RELEASE
        fi
        if [ "$HOST" != "localhost" ] && [ "$FB_SERVER_OS" == "linux" ] ; then
